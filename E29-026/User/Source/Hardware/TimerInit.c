...
//  SetBit(PH_SEL, T4SEL);        //0-不连接到端口GP11；1-连接到端口GP11
/*-------------------------------------------------------------------------------------------------
  时钟分频设置(T2PSC)
  000:cpuclk(24MHz)      001:cpuclk/2^1(12MHz)  010:cpuclk/2^2(6MHz)  011:cpuclk/2^3(3MHz)
  100:cpuclk/2^4(1.5MHz)  101:cpuclk/2^5(750KHz)  110:cpuclk/2^6(375KHz)  111:cpuclk/2^7(187.5KHz)
-------------------------------------------------------------------------------------------------*/
  SetReg(TIM4_CR0,  T4PSC2 | T4PSC1 | T4PSC0, T4PSC2);
...
-------------------------------------------------------------------------------------------------*/
  ...
  SetBit(TIM4_CR1, T4IFE);                                  // 输入有效边沿变化中断使能和基本计数器上溢使能
  ClrBit(TIM4_CR0, T4IRE);                                  // 输入有效边沿变化中断使能和基本计数器上溢使能
/*-------------------------------------------------------------------------------------------------
  定时器2中断优先级配置及芯片中断总使能
  PTIM31-PTIM30，中断优先级控制值从0-3依次表示优先级从最低到最高，共4级优化级控制
  EA,芯片中断总使能
...
-------------------------------------------------------------------------------------------------*/
  TIM4__CNTR = 1;
  TIM4__ARR = 30000;                    //20MS一次
  TIM4__DR = TIM4__ARR >> 1;  
  
/*-----------关闭计数------------------------------------------------*/
  SetBit(TIM4_CR1, T4EN);            //关闭计数器
}
