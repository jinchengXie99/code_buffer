...
typedef struct
{
  ...
  //温度保护
    int16       Temperaturecnt;    //温度保护检测计数
    int16       TemperatureRecCount;//温度保护恢复计数
...
typedef enum
{
    ...
    FaultNtcTemperature = 10,       //发热丝温度保护
    FaultOverTemperature= 11,       //控制板温度保护
} FaultStateType;
...
typedef struct
{
    ...
    uint16 mcADCTemperature;         //板子温度
    
    uint8 HW_ZeroCount;
    uint8 HW_ZeroOldCount;
    uint8 HW_ZeroZ;
    uint8 HW_ZeroT;
    uint8 HW_Gcd;
