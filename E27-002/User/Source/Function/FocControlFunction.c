...
//  }
  ...
  #if (Shunt_Resistor_Mode == Single_Resistor)
  {
    ...
    FOC_TRGDLY = 0x9;                                    // 采样时刻在中点，一般考虑开关噪声影响，会设置延迟；
  ...
  #elif (Shunt_Resistor_Mode == Three_Resistor)            // signel resistor sample
  {
    ...
    FOC_TSMIN  = PWM_DT_LOAD;                              // 死区补偿值
    FOC_TRGDLY =0F;                                        // ADC采样的时刻，采样时刻在计数器零点附近，83为下降沿结束前3个clock采样Ia，与单电阻不同。
                                                          // 01为上升沿开始后第一个clock开始采样。根据实际情况调整。
...
void VariablesPreInit(void)
{
  ...
  KS.FlashWriteOnetimes = 0; //避免欠压重启不记录档位
...
void Motor_Ready(void)
{
   if(McStaSet.SetFlag.CalibFlag==0)
   {
     ...
     mcCurOffset.OffsetCount = 0; //电流采集次数清零
   }
}
/*---------------------------------------------------------------------------*/
...
{
   ClrBit(ADC_MASK_SYSC, CH4EN | CH0EN);// 关闭软件电流采样的ADC
   VariablesPreInit();                           // 电机相关变量初始化
   PI_Init();                                     // PI初始化
  
//   LEDControl();                                    //风筒上电先给转速和温度值   移到这里 避免直接开机到锁定灯没开
}
#endif